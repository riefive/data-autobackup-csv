...
const init = async () => {
  try {
    ...
    Timer.main(downloadProduct, [config.limit], config.interval);
  } catch (err) {
    ...
    throw JSON.stringify(out, null, 4);
  }
};
...
const start = () => {
  ...
  fd.start();
  //Timer.main(function() {console.log('test')}, [config.data.limit], config.interval);
  init();
};
...
const menu = async (key = null) => {
  ...
  switch (key) {
    ...
    default:
      {
        Logger.info('Menjalankan perintah download');
        start();
        break;
      }
  }
